...
async function removeMaid(name){if(!confirm(name+' 님을 명단에서 삭제하시겠습니까?\n배정된 객실에서도 자동으로 배정이 해제됩니다.'))return;showLoad('삭제 중...');const r=await api({action:'removeMaid',name});if(!r.ok){hideLoad();toast('삭제 실패: '+(r.error||''));return;}try{const rr=await api({action:'getRooms'});const rooms=rr.rooms||rr||[];const targets=rooms.filter(function(x){return x.maidName&&x.maidName.split(',').map(function(n){return n.trim().toLowerCase();}).includes(name.toLowerCase());});for(const rm of targets){const remain=rm.maidName.split(',').map(function(n){return n.trim();}).filter(function(n){return n&&n.toLowerCase()!==name.toLowerCase();}).join(', ');await api({action:'assignMaid',roomNo:rm.roomNo,maidName:remain});}if(targets.length){const rr2=await api({action:'getRooms'});S.rooms=rr2.rooms||rr2||[];render();}}catch(e){}hideLoad();toast('✅ '+name+' 삭제완료 (배정 '+'객실 정리 포함)');await refreshMaidList();}
